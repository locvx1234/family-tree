...
- **Chỉnh sửa từng người**: họ tên, giới tính, trạng thái đã mất, năm sinh/mất, số điện thoại, link mạng xã hội, ghi chú.
- **Ảnh đại diện có cắt/căn chỉnh**: kéo và phóng to để lấy đúng khuôn mặt; ảnh được lưu gọn ở kích thước 256px.
- **Xuất file in ấn**: PNG độ phân giải cao và PDF, kèm khung viền + tiêu đề trang trọng.
- **Import / Export JSON** để sao lưu và chia sẻ.
- **Đăng ký / đăng nhập**: mỗi người dùng quản lý các gia phả riêng (JWT + bcrypt), có nút xem/ẩn mật khẩu.
- **Đăng nhập Google (tùy chọn)**: OAuth 2.0/OpenID Connect; nút tự xuất hiện khi máy chủ đã có Google Client ID/Secret.
...
```jsonc
{
  ...
  "persons": {
    "p1": {
      "id": "p1", "name": "…", "gender": "male",
      "birth": "1900", "death": "", "isDeceased": false,
      "phone": "", "socialLinks": "", "note": "", "avatar": null
    }
  },
  "unions": {
...
### Bật đăng nhập bằng Google

Luồng đăng nhập chỉ xin ba scope cơ bản `openid`, `email`, `profile` và không gọi
dịch vụ Google có tính phí. Bạn vẫn cần tạo một OAuth Client loại **Web
application** trong Google Cloud Console, cấu hình OAuth consent screen và thêm
...
Sau đó điền `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` và
`GOOGLE_REDIRECT_URI` vào `.env`, rồi chạy lại:

```bash
docker compose up -d --build
```

Không commit Client Secret vào Git. Với local dev, có thể đăng ký redirect URI
`http://localhost:5173/api/auth/google/callback`; Vite sẽ proxy endpoint này về
Express.
